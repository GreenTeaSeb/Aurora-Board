...
const bricks =
    (options = {}) => {
      ...
      const packed = options.packed.indexOf('data-') === 0
                         ? options.packed
                         : `data-${options.packed}`
      ...
      const selectors = {
        ...
        new : () => toArray(container.children)
                        .filter(node => !node.hasAttribute(`${packed}`))
      }
      ...
      const setup = [ setSizeIndex, setSizeDetail, setColumns ]
          ...
          const run =
              [
                setNodes, setNodesDimensions, setNodesStyles, setContainerStyles
              ]

          // instance

          const instance = knot({pack, update, resize})
      ...
      function getSizeIndex() {
        ...
        return sizes
            .map(size => size.mq &&
                         window.matchMedia(`(min-width: ${size.mq})`).matches)
            .indexOf(true)
      }

      function setSizeIndex() { sizeIndex = getSizeIndex() }
      ...
      function setColumns() { columnHeights = fillArray(sizeDetail.columns) }

      // node helpers

      function setNodes() { nodes = selectors[persist ? 'new' : 'all']() }
      ...
      function setNodesStyles() {
        nodes.forEach((element, index) => {
          ...
          nodeLeft = `${
              ...
              (columnTarget * sizeDetail.gutter)}px`
      ...
      function setContainerStyles() {
        ...
        container.style.width = `${
            ...
            (sizeDetail.columns - 1) * sizeDetail.gutter}px`
